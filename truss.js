...
(function (global) {
  ...
  var TIERS = {
    lite: { key:"lite", name:"Practice", rank:1, mo:650, build:3500, desc:"Core practice. Pursuits, fees, the project spine, the module workflow, calcs and billing.", base:"Single office · up to 6 engineers", includes:["pursuits","proposals","projects","workflow","calcs","billing"] },
    standard: { key:"standard", name:"Firm", rank:2, mo:1400, build:9000, desc:"The working firm. Adds the PE-seal gate, permits, the multiplier board, HR, IT — and the agent org.", base:"Single office · up to 20 engineers", includes:["pursuits","proposals","projects","workflow","calcs","billing","seal","permits","books","hr","it","org"] },
    grandsuite: { key:"grandsuite", name:"Multi-discipline", rank:3, mo:3200, build:16800, desc:"The whole firm, nothing held back. Every department, the full agent org, the Modules engine, and contracts.", base:"Multi-office · unlimited seats · dedicated environment · data migration", includes:["pursuits","proposals","projects","workflow","calcs","billing","seal","permits","modules","books","hr","it","law","org"] }
  };
  var DEPTS = [
  ...
  function renderShell(active) {
    ...
    DEPTS.forEach(function (grp) {
      ...
      grp.items.forEach(function (it) {
        ...
        if (off) { a.title="Add "+ROOMS[it.room].label+" for "+money(ROOMS[it.room].mo)+"/mo"; a.addEventListener("click", function (){ toggleRoom(it.room); toast(ROOMS[it.room].label+" added — "+priceLabel(),"ok"); setTimeout(function(){location.reload();},500); }); }
        nav.appendChild(a);
      });
    });
  ...
  function renderTopbar(crumb) {
    ...
    bar.innerHTML = '<button class="navtoggle" id="navToggle" aria-label="Menu"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M3 12h18M3 18h18"/></svg></button><div class="crumbs">Truss OS · <b>'+esc(crumb)+'</b></div><div class="spacer"></div><div class="tierpill" id="tierPillStatic"><span class="dot"></span><div><b>'+esc(p.tier.name)+(p.changed?' <i class="cfg">configured</i>':'')+'</b> <span class="price">'+money(p.mo)+'/mo licensed</span></div><span class="chev">▾</span></div><div class="who"><div class="av">DW</div><div>Dana Whitfield, PE<br><span class="muted small">Principal · Engineer of Record</span></div></div>';
...
(function (global) {
  ...
  var ID      = "truss";                        /* this product, in the store catalog */
